...
def fetch(params):
    r = requests.get(
        URL, params=params, timeout=30, headers={"User-Agent": "melb-parking-feed/1.0"}
    )
    r.raise_for_status()
...
def extract_latlon(row):
    loc = get_field(
        row, "location", "geo_point_2d", "Location", "geo_shape", "geo_point"
    )
    if not loc:
        return None
    if isinstance(loc, dict):
        ...
        if (
            "coordinates" in loc
            and isinstance(loc["coordinates"], (list, tuple))
            and len(loc["coordinates"]) >= 2
        ):
...
def color_for_status(status):
    s = (status or "").lower()
    if "unoccupied" in s or "free" in s or "available" in s:
        return "green"
    if "present" in s or "occupied" in s or "busy" in s:
        return "red"
    return "orange"
...
def main():
    ...
    def fld(name, default=None):
        return key.get(name.lower(), default or name)
    ...
    # ADD SEARCH BOX for addresses/places
    ...
    cluster = MarkerCluster(
        name="Parking Bays", icon_create_function=icon_create_function
    ).add_to(m)
